...
def ict_concepts(df: pd.DataFrame, ms_length: int = 5, ob_length: int = 10) -> pd.DataFrame:
    """
    ICT Concepts — Python port of LuxAlgo's ICT Concepts Pine v5 indicator.

    Covers:
      - Market Structure Shift (MSS) and Break of Structure (BOS)
      - Fair Value Gaps (FVG) with displacement filter
      - Order Blocks (swing-based, with breaker block detection)
      - Liquidity (clustered equal highs/lows, sweep detection)
      - Volume Imbalance
      - Displacement candles
      - Killzones (NY, London Open, London Close, Asian) — UTC-based
    ...
    # ── ATR(10) for liquidity margin ──────────────────────────────────────
    prev_c = np.roll(close, 1); prev_c[0] = close[0]
    tr     = np.maximum(high - low, np.maximum(np.abs(high - prev_c), np.abs(low - prev_c)))
    atr10  = pd.Series(tr).ewm(span=10, adjust=False).mean().values

    # ── Mean body size for displacement filter ────────────────────────────
    body      = np.abs(close - open_)
    mean_body = pd.Series(body).rolling(ms_length).mean().values
    perc_body = 0.36

    # ══════════════════════════════════════════════════════════════════════
    #  MARKET STRUCTURE SHIFT / BOS
    #  Pine logic: zigzag pivot highs/lows over `ms_length` bars each side.
    #  MSS fires when trend reverses (bear→bull or bull→bear).
    #  BOS fires when trend continues past a prior swing point.
    # ══════════════════════════════════════════════════════════════════════
    mss_bull  = np.zeros(n, dtype=int)
    mss_bear  = np.zeros(n, dtype=int)
    bos_bull  = np.zeros(n, dtype=int)
    bos_bear  = np.zeros(n, dtype=int)
    ms_trend  = np.zeros(n, dtype=int)   # +1 bull / -1 bear

    # Detect pivot highs and lows (confirmed ms_length bars later)
    ph_list = []   # (price, bar_index)
    pl_list = []

    for i in range(ms_length, n - ms_length):
        win_h = high[i - ms_length: i + ms_length + 1]
        win_l = low[i  - ms_length: i + ms_length + 1]
        if len(win_h) < 2 * ms_length + 1:
            continue
        if high[i] == win_h.max() and high[i] > high[i-1] and high[i] > high[i+1]:
            ph_list.append((high[i], i))
        if low[i] == win_l.min() and low[i] < low[i-1] and low[i] < low[i+1]:
            pl_list.append((low[i], i))

    cur_swing_high = np.nan
    cur_swing_low  = np.nan
    trend          = 0
    ph_ptr = pl_ptr = 0
    sh_crossed = sl_crossed = False

    for i in range(n):
        # Confirm pivots once we are ms_length bars past them
        while ph_ptr < len(ph_list) and ph_list[ph_ptr][1] + ms_length <= i:
            cur_swing_high = ph_list[ph_ptr][0]
            sh_crossed     = False
            ph_ptr        += 1
        while pl_ptr < len(pl_list) and pl_list[pl_ptr][1] + ms_length <= i:
            cur_swing_low  = pl_list[pl_ptr][0]
            sl_crossed     = False
            pl_ptr        += 1

        prev_close = close[i-1] if i > 0 else close[i]

        # Bullish cross above swing high
        if not np.isnan(cur_swing_high) and not sh_crossed:
            if prev_close <= cur_swing_high and close[i] > cur_swing_high:
                sh_crossed = True
                if trend <= 0:
                    mss_bull[i] = 1
                    trend       = 1
        ...
        # Bearish cross below swing low
        if not np.isnan(cur_swing_low) and not sl_crossed:
            if prev_close >= cur_swing_low and close[i] < cur_swing_low:
                sl_crossed = True
                if trend >= 0:
                    mss_bear[i] = 1
                    trend       = -1
    ...
    # ══════════════════════════════════════════════════════════════════════
    #  FAIR VALUE GAPS (ICT flavour)
    #  Same 3-bar logic as fair_value_gap() but uses displacement-confirmed
    #  middle candle (L_bodyUP / L_bodyDN in Pine).
    #  Bull: low > high[2] AND close[1] > open[1] (middle bar bullish body)
    #  Bear: high < low[2] AND close[1] < open[1] (middle bar bearish body)
    # ══════════════════════════════════════════════════════════════════════
    ict_fvg_bull    = np.zeros(n, dtype=int)
    ict_fvg_bear    = np.zeros(n, dtype=int)
    ict_fvg_in_bull = np.zeros(n, dtype=int)
    ict_fvg_in_bear = np.zeros(n, dtype=int)

    active_bull_fvg = []   # (top, bot)
    active_bear_fvg = []

    for i in range(2, n):
        # Displacement-confirmed FVG
        mid_bull = close[i-1] > open_[i-1]
        mid_bear = close[i-1] < open_[i-1]

        if low[i] > high[i-2] and mid_bull:
            ict_fvg_bull[i] = 1
            active_bull_fvg.append((low[i], high[i-2]))

        if high[i] < low[i-2] and mid_bear:
            ict_fvg_bear[i] = 1
            active_bear_fvg.append((low[i-2], high[i]))

        # Mitigation: price closed back through the gap
        active_bull_fvg = [(t, b) for (t, b) in active_bull_fvg if close[i] >= b]
        active_bear_fvg = [(t, b) for (t, b) in active_bear_fvg if close[i] <= t]

        for (t, b) in active_bull_fvg:
            if b <= close[i] <= t:
                ict_fvg_in_bull[i] = 1
                break
        for (t, b) in active_bear_fvg:
            if b <= close[i] <= t:
                ict_fvg_in_bear[i] = 1
                break

    # ══════════════════════════════════════════════════════════════════════
    #  ORDER BLOCKS (ICT swing-based)
    #  Pine logic: when close crosses a swing high (bullish move confirmed),
    #  look back through that leg and find the candle with the lowest body
    #  minimum — that candle's body range is the bullish OB.
    #  Breaker: OB is mitigated (price closes through it) but price reverses.
    # ══════════════════════════════════════════════════════════════════════
    ict_ob_in_bull      = np.zeros(n, dtype=int)
    ict_ob_in_bear      = np.zeros(n, dtype=int)
    ict_ob_bull_breaker = np.zeros(n, dtype=int)
    ict_ob_bear_breaker = np.zeros(n, dtype=int)

    mx  = np.maximum(close, open_)   # candle body top
    mn  = np.minimum(close, open_)   # candle body bottom

    # Active OBs: [top, bot, loc, breaker, broken_by_bar]
    active_bull_obs = []
    active_bear_obs = []

    # Re-use pivot lists from MS section
    ph_ptr2 = pl_ptr2 = 0
    confirmed_ph = []
    confirmed_pl = []

    for ph_price, ph_bar in ph_list:
        confirmed_ph.append((ph_price, ph_bar))
    for pl_price, pl_bar in pl_list:
        confirmed_pl.append((pl_price, pl_bar))

    c_ph_ptr = c_pl_ptr = 0

    for i in range(n):
        # Bullish OB: close crossed a swing high
        while c_ph_ptr < len(confirmed_ph) and confirmed_ph[c_ph_ptr][1] + ms_length <= i:
            ph_price, ph_bar = confirmed_ph[c_ph_ptr]
            c_ph_ptr += 1
            # Look back from ph_bar to find lowest body-min candle in that leg
            search_start = max(0, ph_bar - 50)
            best_bar  = ph_bar
            best_mn   = mn[ph_bar]
            for j in range(search_start, ph_bar):
                if mn[j] < best_mn:
                    best_mn  = mn[j]
                    best_bar = j
            ob_top = mx[best_bar]
            ob_bot = mn[best_bar]
            active_bull_obs.append([ob_top, ob_bot, best_bar, False, -1])

        # Bearish OB: close crossed a swing low
        while c_pl_ptr < len(confirmed_pl) and confirmed_pl[c_pl_ptr][1] + ms_length <= i:
            pl_price, pl_bar = confirmed_pl[c_pl_ptr]
            c_pl_ptr += 1
            search_start = max(0, pl_bar - 50)
            best_bar  = pl_bar
            best_mx   = mx[pl_bar]
            for j in range(search_start, pl_bar):
                if mx[j] > best_mx:
                    best_mx  = mx[j]
                    best_bar = j
            ob_top = mx[best_bar]
            ob_bot = mn[best_bar]
            active_bear_obs.append([ob_top, ob_bot, best_bar, False, -1])

        # Check price interaction with bull OBs
        surviving_bull = []
        for ob in active_bull_obs:
            ob_top, ob_bot, loc, breaker, brk_bar = ob
            if not breaker:
                # Mitigation: close below OB bottom
                if mn[i] < ob_bot:
                    ob[3] = True   # becomes breaker
                    ob[4] = i
                    surviving_bull.append(ob)
                else:
                    # Price touching OB
                    if low[i] <= ob_top and low[i] >= ob_bot:
                        ict_ob_in_bull[i] = 1
                    surviving_bull.append(ob)
            else:
                # Breaker: price came back above OB top
                if close[i] > ob_top:
                    ict_ob_bull_breaker[i] = 1
                    surviving_bull.append(ob)
                elif close[i] < ob_bot:
                    pass   # fully broken, discard
                else:
                    surviving_bull.append(ob)
        active_bull_obs = surviving_bull[-ob_length:]   # cap memory
        ...
        for ob in active_bear_obs:
            ...
            if not breaker:
                if mx[i] > ob_top:
                    ob[3] = True
                    ob[4] = i
                    surviving_bear.append(ob)
                else:
                    if high[i] >= ob_bot and high[i] <= ob_top:
                        ict_ob_in_bear[i] = 1
                    surviving_bear.append(ob)
            else:
                if close[i] < ob_bot:
                    ict_ob_bear_breaker[i] = 1
                    surviving_bear.append(ob)
                elif close[i] > ob_top:
                    pass
                else:
                    surviving_bear.append(ob)
        active_bear_obs = surviving_bear[-ob_length:]

    # ══════════════════════════════════════════════════════════════════════
    #  LIQUIDITY  (clustered equal highs / lows + sweep detection)
    #  Pine logic: count zigzag pivots within atr/margin of each new pivot.
    #  If >=3 pivots cluster near same level → liquidity pool.
    #  Sweep: price wick through the pool level, close back inside.
    # ══════════════════════════════════════════════════════════════════════
    ict_liq_buy_swept  = np.zeros(n, dtype=int)
    ict_liq_sell_swept = np.zeros(n, dtype=int)

    margin_div = 4.0   # matches Pine `a = 10 / 4`

    # Use the same ph_list / pl_list from MS section
    active_buy_pools  = []   # (level, bar)
    active_sell_pools = []

    ph_sorted = sorted(ph_list, key=lambda x: x[1])
    pl_sorted = sorted(pl_list, key=lambda x: x[1])

    ph_pool_ptr = pl_pool_ptr = 0

    for i in range(n):
        margin = atr10[i] / margin_div if not np.isnan(atr10[i]) else 0.001

        # Confirm new pivot highs and check for liquidity clusters
        while ph_pool_ptr < len(ph_sorted) and ph_sorted[ph_pool_ptr][1] + ms_length <= i:
            ph_price, ph_bar = ph_sorted[ph_pool_ptr]
            ph_pool_ptr += 1
            count = sum(
                1 for (p, b) in ph_sorted[:ph_pool_ptr]
                if abs(p - ph_price) < margin
            )
            if count >= 3:
                active_buy_pools.append((ph_price, ph_bar))

        while pl_pool_ptr < len(pl_sorted) and pl_sorted[pl_pool_ptr][1] + ms_length <= i:
            pl_price, pl_bar = pl_sorted[pl_pool_ptr]
            pl_pool_ptr += 1
            count = sum(
                1 for (p, b) in pl_sorted[:pl_pool_ptr]
                if abs(p - pl_price) < margin
            )
            if count >= 3:
                active_sell_pools.append((pl_price, pl_bar))

        # Sweep detection: wick through, close back
        new_buy_pools = []
        for (level, bar) in active_buy_pools:
            if high[i] > level and close[i] < level:
                ict_liq_buy_swept[i] = 1   # stop-hunt above, closed back below
            else:
                new_buy_pools.append((level, bar))
        active_buy_pools = new_buy_pools[-20:]

        new_sell_pools = []
        for (level, bar) in active_sell_pools:
            if low[i] < level and close[i] > level:
                ict_liq_sell_swept[i] = 1  # stop-hunt below, closed back above
            else:
                new_sell_pools.append((level, bar))
        active_sell_pools = new_sell_pools[-20:]

    # ══════════════════════════════════════════════════════════════════════
    #  VOLUME IMBALANCE
    #  Pine: vImb_Bl = open > close[1] AND high[1] > low AND close > close[1]
    #                 AND open > open[1] AND high[1] < min(close,open)  [body gap]
    # ══════════════════════════════════════════════════════════════════════
    ict_vim_bull = np.zeros(n, dtype=int)
    ict_vim_bear = np.zeros(n, dtype=int)

    for i in range(1, n):
        # Bull VI: current open gapped above prior candle body, bodies don't overlap
        if (open_[i] > close[i-1] and
            high[i-1] > low[i] and
        ...
        # Bear VI: current open gapped below prior candle body
        if (open_[i] < close[i-1] and
            low[i-1] < high[i] and
            close[i] < close[i-1] and
            open_[i] < open_[i-1] and
    ...
    # ══════════════════════════════════════════════════════════════════════
    #  DISPLACEMENT
    #  Pine: large body (> mean body * 1.0), minimal wicks (< body * 0.36)
    # ══════════════════════════════════════════════════════════════════════
    ict_disp_up = np.zeros(n, dtype=int)
    ict_disp_dn = np.zeros(n, dtype=int)

    for i in range(ms_length, n):
        if np.isnan(mean_body[i]):
            continue
        b = body[i]
        upper_wick = high[i] - mx[i]
        lower_wick = mn[i]   - low[i]
        large      = b > mean_body[i]
        small_wick = upper_wick < b * perc_body and lower_wick < b * perc_body
        if large and small_wick:
            if close[i] > open_[i]:
                ict_disp_up[i] = 1
            else:
                ict_disp_dn[i] = 1
    ...
    # ══════════════════════════════════════════════════════════════════════
    #  KILLZONES (UTC times matching Pine sessions)
    #  NY Open:        12:00–14:00 UTC  (07:00–09:00 America/New_York EST)
    #  London Open:    07:00–10:00 UTC
    #  London Close:   15:00–17:00 UTC
    #  Asian:          00:00–04:00 UTC  (10:00–14:00 Asia/Tokyo = 01:00–05:00 UTC approx)
    # ══════════════════════════════════════════════════════════════════════
    ict_kz_ny      = np.zeros(n, dtype=int)
    ict_kz_ldn_o   = np.zeros(n, dtype=int)
    ict_kz_ldn_c   = np.zeros(n, dtype=int)
    ict_kz_asian   = np.zeros(n, dtype=int)
    ict_nwog       = np.zeros(n, dtype=int)
    ict_ndog       = np.zeros(n, dtype=int)
    ict_in_nwog    = np.zeros(n, dtype=int)
    ict_in_ndog    = np.zeros(n, dtype=int)

    # Try to get timestamps from index
    idx = df.index
    has_time = hasattr(idx, 'hour')

    if has_time:
        hours   = idx.hour.values
        minutes = idx.minute.values
        hm      = hours * 100 + minutes   # HHMM integer for fast comparison

        # NY Open: 1200–1400 UTC
        ict_kz_ny    = ((hm >= 1200) & (hm < 1400)).astype(int)
        # London Open: 0700–1000 UTC
        ict_kz_ldn_o = ((hm >= 700)  & (hm < 1000)).astype(int)
        # London Close: 1500–1700 UTC
        ict_kz_ldn_c = ((hm >= 1500) & (hm < 1700)).astype(int)
        # Asian: 0000–0400 UTC
        ict_kz_asian = ((hm >= 0)    & (hm < 400)).astype(int)

        # ── NWOG / NDOG ───────────────────────────────────────────────────
        # NWOG: gap between Friday close and Monday open
        # NDOG: gap between previous day close and current day open
        weekdays = idx.weekday.values   # 0=Mon … 4=Fri
        fri_close_price = np.nan
        fri_close_bar   = -1
        nwog_top = nwog_bot = np.nan

        prev_day_close  = np.nan
        prev_day_bar    = -1
        ndog_top = ndog_bot = np.nan
        ...
        for i in range(1, n):
            # NDOG: new day started
            if hasattr(idx, 'date') and idx[i].date() != idx[i-1].date():
                prev_day_close = close[i-1]
                cur_day_open   = open_[i]
                if not np.isnan(prev_day_close):
                    ndog_top = max(prev_day_close, cur_day_open)
                    ndog_bot = min(prev_day_close, cur_day_open)

            if not np.isnan(ndog_top) and ndog_top != ndog_bot:
                if ndog_bot <= close[i] <= ndog_top:
                    ict_in_ndog[i] = 1
                ict_ndog[i] = 1

            # NWOG: Friday → Monday gap
            if weekdays[i-1] == 4 and weekdays[i] == 0:   # Fri close → Mon open
                fri_close_price = close[i-1]
                mon_open_price  = open_[i]
                nwog_top = max(fri_close_price, mon_open_price)
                nwog_bot = min(fri_close_price, mon_open_price)

            if not np.isnan(nwog_top) and nwog_top != nwog_bot:
                if nwog_bot <= close[i] <= nwog_top:
                    ict_in_nwog[i] = 1
                ict_nwog[i] = 1
    ...
    df["ict_mss_bull"]         = pd.Series(mss_bull,        index=idx_df)
    df["ict_mss_bear"]         = pd.Series(mss_bear,        index=idx_df)
    df["ict_bos_bull"]         = pd.Series(bos_bull,        index=idx_df)
    df["ict_bos_bear"]         = pd.Series(bos_bear,        index=idx_df)
    df["ict_ms_trend"]         = pd.Series(ms_trend,        index=idx_df)
    df["ict_fvg_bull"]         = pd.Series(ict_fvg_bull,    index=idx_df)
    df["ict_fvg_bear"]         = pd.Series(ict_fvg_bear,    index=idx_df)
    df["ict_fvg_in_bull"]      = pd.Series(ict_fvg_in_bull, index=idx_df)
    df["ict_fvg_in_bear"]      = pd.Series(ict_fvg_in_bear, index=idx_df)
    df["ict_ob_in_bull"]       = pd.Series(ict_ob_in_bull,      index=idx_df)
    df["ict_ob_in_bear"]       = pd.Series(ict_ob_in_bear,      index=idx_df)
    df["ict_ob_bull_breaker"]  = pd.Series(ict_ob_bull_breaker, index=idx_df)
    df["ict_ob_bear_breaker"]  = pd.Series(ict_ob_bear_breaker, index=idx_df)
    df["ict_liq_buy_swept"]    = pd.Series(ict_liq_buy_swept,   index=idx_df)
    df["ict_liq_sell_swept"]   = pd.Series(ict_liq_sell_swept,  index=idx_df)
    df["ict_vim_bull"]         = pd.Series(ict_vim_bull,     index=idx_df)
    df["ict_vim_bear"]         = pd.Series(ict_vim_bear,     index=idx_df)
    df["ict_displacement_up"]  = pd.Series(ict_disp_up,     index=idx_df)
    df["ict_displacement_dn"]  = pd.Series(ict_disp_dn,     index=idx_df)
    df["ict_kz_ny"]            = pd.Series(ict_kz_ny,       index=idx_df)
    df["ict_kz_london_open"]   = pd.Series(ict_kz_ldn_o,    index=idx_df)
    df["ict_kz_london_close"]  = pd.Series(ict_kz_ldn_c,    index=idx_df)
    df["ict_kz_asian"]         = pd.Series(ict_kz_asian,    index=idx_df)
    df["ict_nwog_present"]     = pd.Series(ict_nwog,        index=idx_df)
    df["ict_ndog_present"]     = pd.Series(ict_ndog,        index=idx_df)
    df["ict_price_in_nwog"]    = pd.Series(ict_in_nwog,     index=idx_df)
    df["ict_price_in_ndog"]    = pd.Series(ict_in_ndog,     index=idx_df)
...
def feature_engineer(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── ICT Concepts ──────────────────────────────────────────────────────────
    df = ict_concepts(df, ms_length=5, ob_length=10)
...
def run(
    ...
    # ── 7. Save ────────────────────────────────────────────────────────
    source_stem = Path(csv_path).stem
    source_parts = source_stem.rsplit("_", 1)
    symbol = source_parts[0] if len(source_parts) == 2 else source_stem
    timeframe = source_parts[1] if len(source_parts) == 2 else ""
